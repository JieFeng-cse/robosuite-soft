...
class LiftCloth(SingleArmEnv):
    ...
    def _initialize_cloth_pos(self):
        ...
        self.sim.data.xfrc_applied[mid1_id, :3] = np.random.uniform(-1,0,size=3)
        self.sim.data.xfrc_applied[mid2_id, :3] = np.random.uniform(-1,0,size=3)

        # Force the corners move randomly
        # self.sim.data.xfrc_applied[self.corner_id, :3] = np.random.uniform(-.3,.3,size=3)
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            sensors = [cloth_pos, corner_pos, gripper_to_cloth_pos]
